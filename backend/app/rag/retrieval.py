...
def link_entities(query: str, exclude: set[str] | None = None) -> list[str]:
    ...
    return [hit["id"] for hit in hits if hit["id"] not in exclude]
...
def retrieve(query: str, exclude_entities: set[str] | None = None) -> RetrievalResult:
    ...
    expanded in earlier dialogue turns, reducing redundant retrieval.
    """
    seed_ids = link_entities(query, exclude=exclude_entities)
